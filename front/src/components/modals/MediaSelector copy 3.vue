...
const handleUpload = async (event: Event) => {
  ...
  try {
    ...
    // Limpiamos el input de forma segura
...
const deleteImage = async (id: number) => {
    // Ya no necesitamos event.stopPropagation() aquí porque usamos @click.stop en el HTML
...
<template>
  <div class="modal fade show d-block modal-entrada" style="background: rgba(0,0,0,0.85); z-index: 1050;" @click.self="$emit('close')">
        ...
        <div class="bg-white p-3 border-bottom sticky-top" style="z-index: 10;">
            <div class="input-group">
                ...
                <input 
                  v-model="searchQuery" 
                  type="text" 
                  class="form-control border-start-0 bg-light focus-ring-none" 
          ...
          <div v-else class="row g-3">
            <div v-for="img in filteredLibrary" :key="img.id" class="col-xl-2 col-lg-3 col-md-4 col-6">
              ...
              <div class="media-card card h-100 border-0 shadow-sm position-relative group">
                ...
                <div @click="$emit('select', img.url || img.path || '')" class="h-100 d-flex flex-column">
