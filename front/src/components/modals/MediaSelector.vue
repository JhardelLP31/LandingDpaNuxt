...
// ID EFECTIVO: Prioridad Prop -> Store (Pinia) -> Default 1
...
const loadMedia = async () => {
    ...
    try {
        ...
        // 2. Vemos exactamente qué estructura nos mandó Laravel
...
const handleUpload = async (event: Event) => {
  ...
  if (file.size > 5 * 1024 * 1024) {
    Swal.fire({
        ...
        text: "El peso máximo permitido es de 5MB",
        icon: "warning",
        didOpen: () => {
  ...
  try {
    ...
    Swal.fire({
        title: "Error 500",
        text: "Hubo un problema en el servidor al subir el archivo",
        icon: "error",
        didOpen: () => {
            // ¡Esto pone la alerta por encima del modal negro!
            const container = Swal.getContainer();
            if (container) container.style.zIndex = "3000"; 
        }
    });
...
const createFolder = async () => {
    ...
    if (name) {
        try {
            ...
            // 1. Enviamos la petición y capturamos la respuesta entera para verla
            ...
            // 3. Recargamos la vista de carpetas
            await loadMedia();

            // 4. Mostramos una alerta de éxito
            Swal.fire({
                toast: true,
                position: 'top-end',
                icon: 'success',
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable shadow-lg">
      <div class="modal-content border-0 rounded-3 overflow-hidden">
        ...
        <div class="modal-body bg-light p-4">
          <div v-if="!effectiveUnitId" class="alert alert-warning text-center">
