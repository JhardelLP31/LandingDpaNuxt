...
// Listas para menús
const menus_list = ref<any[]>([]); // La lista para el selector (dropdown)
const menu_tree = ref<any[]>([]);  // Los ítems reales que se ven en la cabecera
...
const form = reactive({
    ...
    accent_color: '#ffcc00',
    selected_menu_id: null as number | null, // ID del menú en la tabla principal
    header_config: getInitialBrandingData('HeaderOverlay'),
    custom_css: '' // Aquí empaquetamos el JSON
});
...
// 1. Cargar los items del menú seleccionado
const loadMenuItems = async () => {
    ...
    try {
        ...
        console.error("Error al cargar items del menú", e);
    }
};

// 2. Carga inicial de todo el branding
const loadInitialData = async () => {
    loading.value = true;
    try {
        // Cargar lista de menús disponibles para la unidad académica
        ...
        // Cargar datos de branding existentes
        ...
        if (data) {
            ...
            // Sincronizar el menú seleccionado
            form.selected_menu_id = data.selected_menu_id;
            
            // DESEMPAQUETAR EL JSON (custom_css)
            if (data.custom_css) {
                try {
                    const parsedBlocks = JSON.parse(data.custom_css);
                    if (Array.isArray(parsedBlocks) && parsedBlocks.length > 0) {
                        const savedHeader = parsedBlocks[0]; // El primer bloque es el Header
                        ...
                        // Si el menu_id está dentro del config pero no en la columna, lo recuperamos
                        if (!form.selected_menu_id && form.header_config.menu_id) {
                            form.selected_menu_id = form.header_config.menu_id;
                        }
                    }
                } catch (err) {
                    console.error("Error parseando diseño guardado, usando valores por defecto");
                }
            }

            // Una vez cargado el ID del menú, traemos sus ítems para la vista previa
            if (form.selected_menu_id) {
                await loadMenuItems();
            }
        }
    } catch (e) {
        console.log("No hay configuración previa, iniciando limpio");
    } finally {
        loading.value = false;
    }
};
...
const saveBranding = async () => {
    loading.value = true;
    try {
        // 1. Inyectar el menu_id actual en el config antes de guardar
        form.header_config.menu_id = form.selected_menu_id;
        
        // 2. Empaquetar el bloque de diseño para el campo custom_css
        const headerBlock = [{
        ...
        Swal.fire({ 
            ...
            title: '¡Cambios publicados con éxito!', showConfirmButton: false, timer: 2000 
        });
    } catch (e: any) {
