...
import { useRouter } from "vue-router"; // <--- Importante para redireccionar sin recargar
...
const alerta = (title: string, text: string, icon: 'success' | 'error' | 'warning') => {
  return Swal.fire({ title, text, icon, confirmButtonColor: '#0f6eb6' });
};

// 2. Variables
const credentials = reactive({
  username: "jpumahuillca",
  password: "",
});
...
const handleLogin = async () => {
  ...
  if (result) {
    ...
    try {
      // Preparamos payload (mapeando username a email si es necesario)
      const payload = {
        email: credentials.username, 
        password: credentials.password
      };

      // Llamada al Backend (LoginController.php)
      const res: AxiosResponse = await HttpClient.post('login-system', payload);
      
      console.log("Respuesta Backend:", res.data); // Debug para ver qué llega

      if (res.data.access_token) {
        
        // --- CORRECCIÓN CRÍTICA AQUÍ ---
        // El backend devuelve role y permissions DENTRO de 'user'.
        // Al hacer spread (...res.data.user), ya estamos pasando role y permissions.
        
        const sessionData = {
            ...res.data.user,             // Incluye full_name, avatar, role, permissions
            token: res.data.access_token  // Agregamos el token
        };

        // Guardamos en Pinia/LocalStorage
        useAuth.saveSession(sessionData);

        // Alerta de éxito opcional (o redirección directa)
        // alerta('Bienvenido', `Hola ${res.data.user.full_name}`, 'success');

        // --- REDIRECCIÓN ---
        // Usamos replace para que no pueda volver atrás con el botón del navegador
        // Si tu ruta principal es '/', usa esa. Si es '/cms/dashboard-kpi', usa esa.
        setTimeout(() => {
       ...
       const errorMsg = e.response?.data?.error || e.response?.data?.message || 'Error desconocido';

       if (status === 401) {
          alerta('Credenciales Incorrectas', 'El usuario o contraseña no son válidos.', 'error');
       } else if (status === 403) {
          alerta('Acceso Denegado', errorMsg, 'warning');
       } else {
          alerta('Error de Sistema', 'No se pudo conectar con el servidor.', 'error'); 
       }
