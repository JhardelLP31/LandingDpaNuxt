...
const loadMedia = async () => {
    ...
    try {
        ...
        // 🔥 2. Aplicado al cargar
        handleApiError(error, "No se pudo cargar la galería");
    } finally {
        loading.value = false;
    }
};
...
const handleUpload = async (event: Event) => {
  ...
  try {
    ...
    // 🔥 3. Aplicado al subir archivo
    handleApiError(e, "Hubo un problema al subir el archivo");
  } finally {
...
const createFolder = async () => {
    ...
    if (name) {
        try {
            ...
            // 🔥 4. Aplicado al crear carpeta
            handleApiError(e, "No se pudo crear la carpeta");
        }
    }
};

// --- EDITAR (RENOMBRAR) CARPETA ---
const renameFolder = async (folder: MediaFolder) => {
    ...
    if (newName && newName.trim() !== '' && newName !== folder.name) {
        try {
            await HttpClient.put(`media-folders/${folder.id}`, {
                name: newName
            });
            await loadMedia(); 
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Nombre actualizado', showConfirmButton: false, timer: 2000 });
        } catch (e: any) {
            // 🔥 5. Aplicado al renombrar carpeta
            handleApiError(e, "No se pudo renombrar la carpeta");
        }
    }
};
...
const deleteFile = async (item: MediaItem) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            // 🔥 6. Aplicado al eliminar archivo
            handleApiError(error, "No se pudo eliminar el archivo");
        }
    }
};
...
const deleteFolder = async (folder: MediaFolder) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            // 🔥 7. Aplicado al eliminar carpeta (Si por ejemplo no está vacía y Laravel devuelve 422 o 500)
            handleApiError(error, "No se pudo eliminar la carpeta");
        }
    }
};
    ...
    <div class="card shadow-sm border-0 rounded-3 overflow-hidden" style="min-height: 80vh;">
      ...
      <div class="card-header bg-white border-bottom py-3 d-flex flex-wrap align-items-center justify-content-between gap-3">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h5 class="mb-0 fw-bold text-dark">Gestor de Archivos</h5>
                <small class="text-muted">Administra los recursos de tu Unidad Académica</small>
            </div>
        </div>
            ...
            <div v-else class="p-4 flex-grow-1 overflow-auto">
                ...
                <div class="row g-3">
                    <div v-for="folder in filteredFolders" :key="'f-'+folder.id" class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                        <div class="card h-100 folder-card border border-light shadow-sm position-relative group-hover cursor-pointer" @click="openFolder(folder)">
                            
                            <div class="position-absolute top-0 end-0 m-2 d-flex gap-1 z-index-2">
                                <button class="btn btn-sm btn-light text-primary shadow-sm rounded-circle d-flex align-items-center justify-content-center action-btn" 
                                        @click.stop="renameFolder(folder)" title="Renombrar">
                                    <i class="las la-pen"></i>
                                </button>
                                <button class="btn btn-sm btn-light text-danger shadow-sm rounded-circle d-flex align-items-center justify-content-center action-btn" 
                                        @click.stop="deleteFolder(folder)" title="Eliminar">
                                    <i class="las la-trash-alt"></i>
                                </button>
                            </div>
                            
                            <div class="card-body text-center p-4">
                                <i class="las la-folder text-warning display-3 mb-2 mt-2"></i>
                    ...
                    <div v-for="item in filteredFiles" :key="'m-'+item.id" class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                        <div class="card h-100 media-card border border-light shadow-sm position-relative group-hover cursor-pointer" @click="copyUrl(item.url)">
                            ...
                            <div class="img-container bg-light d-flex align-items-center justify-content-center border-bottom rounded-top" 
                ...
                <div v-if="!loading && filteredFiles.length === 0 && filteredFolders.length === 0" class="d-flex flex-column align-items-center justify-content-center py-5 h-100">
                    ...
                    <p class="text-muted">Haz clic en "Subir" para agregar archivos.</p>
                </div>
