...
// Interfaces para evitar errores de TS
interface MenuItem {
  id: string | number;
  title: string;
  link: string;
...
// Tu lógica de procesamiento intacta pero optimizada
        ...
        if (newItem.type === 'dynamic_group') {
            newItem.children = allCategories.value
                ...
                .map(cat => ({
                    id: 'cat-' + cat.id,
                    title: cat.name,
                    //link: newItem.slug === 'course' ? `/course/${cat.slug}` : `/${cat.slug}`,
...
// Tu función buildTreeSafe original
