...
// 🔧 CONFIGURACIÓN: Tu dominio de Laravel
const BACKEND_URL = import.meta.env.VITE_API_URL
// Definición de Props con Tipado Seguro
const props = defineProps<{
  data: {
    title: string;
    link: string;
    image: string | null; // Aceptamos null
    status_label?: string; 
    startDate?: string;
    duration?: string;
...
const resolveImage = (path: string | null | undefined, isAvatar = false) => {
  // 1. Si es null, undefined o string vacío -> Placeholder Online
  if (!path) {
    return isAvatar 
  ...
  // 2. Si es el placeholder local que te da error 404 -> Placeholder Online
  if (path.includes('placeholder-course.jpg')) {
      return 'https://placehold.co/600x400?text=Curso+Disponible';
  }

  // 3. Si ya es una URL completa (http...), la dejamos tal cual
  if (path.startsWith('http')) {
    return path;
  }

  // 4. EL FIX CLAVE: Si es ruta relativa de Laravel (/storage/...), le pegamos el dominio
  if (path.startsWith('/storage')) {
    // Retorna: http://127.0.0.1:8000/storage/pages/foto.png
...
// Manejador de error por si la imagen existe pero está corrupta
