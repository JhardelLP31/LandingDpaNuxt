...
<template>
  <section 
    v-if="data && data.counters.length > 0"
    ref="sectionRef"
    class="theme-counter" 
    :style="{ 
...
.theme-counter {
  ...
  display: flex;
  align-items: center;
  background-color: $esan-white; /* Fondo blanco */
}

.overlay {
  background: none; /* Eliminamos el overlay oscuro */
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.85) 0%, rgba(255, 255, 255, 0.919) 100%);
...
.content-wrapper {
  text-align: center;
  color: $esan-black; /* Texto principal negro */
}

.subtitle {
  color: $esan-red; /* Se mantiene rojo */
  font-weight: 700;
  letter-spacing: 3px;
  margin-bottom: 10px;
...
.number-wrapper {
  ...
  font-weight: 900;
  line-height: 1;
  color: $esan-red; /* Números en rojo */
  margin-bottom: 15px;
}
...
.description {
  font-size: 1.1rem;
  font-weight: 300;
  color: $esan-dark-gray; /* Descripción en gris oscuro */
  max-width: 280px;
  margin: 0 auto 40px;
  line-height: 1.5;
}
