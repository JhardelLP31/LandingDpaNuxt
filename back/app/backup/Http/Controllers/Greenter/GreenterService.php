...
class GreenterService
{
    ...
    public function getClient($sale)
    {
        ...
        if($sale->is_exportacion == 1){
            ...
            return (new Client())
                ->setTipoDoc($cod_document) // DNI - Catalog. 06
    ...
    public function getInvoice($data,$company,$sale)
    {
        ...
        if($sale->retencion_igv > 0){
            if($sale->retencion_igv == 1){//Retención
                $invoice->setDescuentos([
                    (new Charge())
    ...
    public function getDetails($sale_details)
    {
        ...
        foreach ($sale_details as $sale_detail) {
            ...
            $detail = (new SaleDetail())
                ->setCodProducto($sale_detail->product->cod_producto ?? 'P001')
                ->setDescripcion($sale_detail->product->title)
                ->setUnidad($sale_detail->product->unidad_medida) // Unidad - Catalog. 03
                ...
                ->setPorcentajeIgv($porcentaje_igv) // 18% o 0%
                ->setIgv($igv)
                ->setTipAfeIgv($sale_detail->tip_afe_igv)
                ->setTotalImpuestos($total_impuesto)
            ...
            array_push($green_sale_details,$detail);
        }
        return $green_sale_details;
    }
    ...
    public function getNote($data,$company,$note_electronic)
    {
        $invoice = (new Note())
            ->setUblVersion('2.1')
            // ->setTipoOperacion($data['tipo_operacion']) // Venta - Catalog. 51
            ...
            ->setTipoMoneda($data['tipo_moneda']) // Sol - Catalog. 02
            ->setCompany($this->getCompany($company))
            ->setClient($this->getClient($note_electronic));
        ...
        if($note_electronic->retencion_igv > 0){
            if($note_electronic->retencion_igv == 3){//PERCEPCION
                $invoice->setPerception((new SalePerception())
                ->setCodReg('51')
                ->setPorcentaje(0.04)
                ->setMtoBase($data['mto_imp_venta'])
        ...
        //Mto Operaciones
        if($note_electronic->is_exportacion == 1){
            $invoice->setMtoOperExportacion($data['mto_oper_exportacion'] ?? null);
        }else{
            $invoice->setMtoOperGravadas($data['mto_oper_gravadas'] ?? null)
                ->setMtoOperExoneradas($data['mto_oper_exoneradas'] ?? null)
        ...
        //Productos
        ->setDetails($this->getDetails($note_electronic->electronic_note_details))
