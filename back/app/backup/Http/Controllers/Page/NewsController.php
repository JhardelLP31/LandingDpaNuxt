...
class NewsController extends Controller
{
    ...
    public function getNewsByCategory($identifier)
    {
        // 1. Buscamos la categoría (detectando si es ID o Slug)
        ...
        // 2. Traemos todas las noticias de esa categoría
        $news = Page::where('category_id', $category->id)
                    ->orderBy('created_at', 'desc') // Las más nuevas primero
                    ->get();

        // 3. Retornamos todo
        return response()->json([
            'category' => $category, // Para mostrar el nombre en el título
            'news'     => $news      // La lista de noticias
        ]);
    }
