...
const mappedPosts = computed<Post[]>(() => {
  ...
  return posts.map(post => {
    ...
    let date = post.start_date ? new Date(post.start_date).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' }) : '';
  ...
  <div class="bento-section">
    <div class="container">
      ...
      <div class="d-flex justify-content-between align-items-end mb-4 border-bottom pb-3" style="border-color: #eee;">
        <div>
            <span class="text-uppercase fw-bold text-muted small ls-2 mb-1 d-block">
                {{ subtitle }}
            </span>
            <h2 class="display-4 fw-bold m-0 lh-1 esan-title">{{ title }}</h2>
        </div>
        
        <!--<router-link to="/blog" class="link-arrow d-none d-md-block">
            Ver archivo completo <i class="las la-arrow-right"></i>
        </router-link>-->
    </div>
