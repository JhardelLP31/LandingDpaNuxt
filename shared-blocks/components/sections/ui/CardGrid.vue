...
.std-card {
  ...
  // === HOVER EFFECT (LA MAGIA) ===
  &:hover {
    // 1. La tarjeta sube
    transform: translateY(-10px);
    
    // 2. La sombra toma el color de la categoría (Glow Effect)
    box-shadow: 0 20px 40px -10px var(--theme-color); 
    
    // 3. El borde se pinta del color
    ...
    .card-title {
      color: var(--theme-color); // Título se pinta del color
    ...
    .color-bar {
      height: 6px; // La barra inferior crece un poco
    }
...
.card-info {
  ...
  display: flex;
  flex-direction: column;
  flex-grow: 1; // Empuja el footer hacia abajo si hay espacio
