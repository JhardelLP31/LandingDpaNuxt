...
const localForm = ref({
  // 1. VARIABLES VISUALES DEL HERO (Nuevas)
  ...
  // 2. TEXTOS (Nombres ajustados para coincidir con HeroFormSection.vue)
  ...
  marketingTitle: props.modelValue?.marketingTitle || 'INVIERTE EN TU EXCELENCIA', // El texto oculto SEO

  // 3. VARIABLES DEL FORMULARIO INTERNO
  campaign_id: props.modelValue?.campaign_id || null,
  cod_form_old: props.modelValue?.cod_form_old || '', // <-- NUEVO CAMPO AÑADIDO
...
// Sincroniza los cambios con el componente padre (Page Builder)
...
// --- LÓGICA DE CURSOS / PROGRAMAS DE ESTUDIO ---
