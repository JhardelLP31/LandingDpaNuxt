...
// Instanciamos reCAPTCHA.
// En Nuxt será funcional (gracias al plugin). En el Admin SPA será undefined.
...
const submitForm = async () => {
  ...
  try {
    // --- LÓGICA HÍBRIDA DE RECAPTCHA ---
    let recaptchaToken = 'admin_bypass'; // Token por defecto si estamos en el Admin Builder

    if (recaptcha) {
      // Estamos en Nuxt (Vista Pública): Generamos token real
      await recaptcha.recaptchaLoaded();
      recaptchaToken = await recaptcha.executeRecaptcha('submit_lead');
    } else {
      console.warn('🔧 Modo Admin Builder: Saltando validación de reCAPTCHA');
    }
    ...
    const payloadForDB = {
      ...
      // Cambiamos aquí: Enviamos los apellidos por separado en la raíz del request
      apellido_paterno: formData.value.apellido_paterno.trim(),
      apellido_materno: formData.value.apellido_materno.trim(),
      ...
      payload: {
        ...
        acepta_condiciones: formData.value.acepta_condiciones,
        acepta_publicidad: formData.value.acepta_publicidad,
        url_slug: currentSlug,
        cod_form_old: props.config.cod_form_old || 0 
      }
    };
    ...
    /*formData.value = {
      ...
      acepta_condiciones: false, acepta_publicidad: false, condition:'https://www.ue.edu.pe/pregrado/politica-de-privacidad', cod_form_old: props.config.cod_form_old || 0
    };*/
...
<template>
  <form id="comunicate" @submit.prevent="submitForm" class="lead-form">
    ...
    <div class="row g-3 mb-3">
      <input type="hidden" v-model="formData.cod_form_old">
    ...
    <div class="col-12 mb-3">
      <select class="form-select" v-model="formData.estudios_superiores" required :class="{ 'text-placeholder': formData.estudios_superiores === '' }">
        <option value="" disabled>Estudios superiores...</option>
    ...
    <div v-if="config.show_courses && config.courses_list && config.courses_list.length > 0" class="col-12 mb-3">
      <select class="form-select" v-model="formData.curso_seleccionado" required :class="{ 'text-placeholder': formData.curso_seleccionado === '' }">
        <option value="" disabled>Carrera de interés...</option>
