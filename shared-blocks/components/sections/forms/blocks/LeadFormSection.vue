...
// Separamos los colores para el Botón y para los Enlaces
const themeColor = computed(() => props.data?.themeColor || '#a00000'); // Color del Botón (ej. Guinda)
const linkColor = computed(() => props.data?.linkColor || '#ff2b2b');   // Color del Link (ej. Rojo claro)

// Textos sobre el Formulario (H2 visible)
const mainFormTitle = computed(() => props.data?.mainTitle);
const mainFormSubtitle = computed(() => props.data?.subTitle);

// Texto de Marketing e Imagen (H1 y Graduado)
const marketingTitle = computed(() => props.data?.marketingTitle || 'INVIERTE EN TU EXCELENCIA');
const marketingImage = computed(() => props.data?.marketingImage || ''); // Opcional, arranca vacío

// Configuración del Formulario (Ajustado por si el editor manda todo en un nivel plano)
