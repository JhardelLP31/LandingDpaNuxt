...
// Recibe modelValue para los datos y availableSections desde el Page Builder
...
// Estado para el panel de importación rápida
const showSectionPicker = ref(false);

// Sincronización: Prop -> Local
...
const importSection = (section: { id: string, title: string }) => {
  localData.value.menuItems.push({ 
    ...
    url: `#sec-${section.id}` // Coincide con el ID generado en el canvas
  });
};
</script>
