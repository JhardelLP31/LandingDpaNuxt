...
// Navegación cuando presionan Enter
const goToFullSearch = () => {
  if (search.value.length >= 2) {
    router.push({ name: 'public.search', query: { q: search.value } });
    results.value = []; // Cerramos el dropdown
...
.top-bar {
  background-color: #1a4b70; // El azul de ESAN
  color: white;
  min-height: 40px; // Altura controlada
}

.search-wrapper {
  position: relative;
  width: 100%;
  max-width: 300px; // Buscador más pequeño
}
