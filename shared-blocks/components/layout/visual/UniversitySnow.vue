...
// Tokens temáticos de programación y sistemas
const tokens = ['{ }', '01', '</>', 'if()', '=>', 'git', 'sudo', '[]', '&&', '1010' ];

const getRandomStyles = (index: number) => {
  const left = Math.floor(Math.random() * 100);
  const duration = 5 + Math.random() * 10; // Más rápido para dar sensación de flujo de datos
  const delay = Math.random() * -15;
  const size = 0.9 + Math.random() * 1.2;
  const opacity = 0.1 + Math.random() * 0.4; // Variamos opacidad para dar profundidad
