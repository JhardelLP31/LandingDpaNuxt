...
// --- STATE ---
const activeItem = ref<SectorItem | null>(null); // Inicia en null para estado "General"
const courses = ref<Course[]>([]);
const loading = ref(false);

// --- LOGIC MAPPING (Reutilizable) ---
...
const resetToGeneral = () => {
    if (activeItem.value === null) return; // Ya estamos en general
    activeItem.value = null;
    fetchInitialCourses();
    scrollToStage();
};

// Función para centrar la vista cuando cambian los cursos
const scrollToStage = () => {
...
// Actualizamos el handleSelect para que también haga scroll
...
onMounted(() => {
    fetchInitialCourses(); // Al iniciar, carga el mix global
});
...
// NAVEGACIÓN RESPONSIVA (Ajuste de scroll y visualización)
// NAVEGACIÓN QUE SALTA DE LÍNEA (WRAP)
.grid-controls {
    display: flex;
    flex-wrap: wrap;           // ESTO hace que salten de línea
    justify-content: center;   // Centra las categorías si sobran
...
.control-item {
    ...
    &:hover:not(.is-active) {
        opacity: 0.8;
        transform: translateY(-4px); // Se eleva un poquito
        
        .icon-box {
            background: #fff;
            border-color: #cbd5e1; // Color de borde suave al pasar el mouse
            box-shadow: 0 6px 12px rgba(0, 0, 0, 0.05);
            
            .control-img {
                transform: scale(1.05); // La imagen crece un 5%
            }
        }
    }

    // --- ESTADO ACTIVO (Mantiene lo que ya tenías) ---
    &.is-active {
        opacity: 1;
        transform: translateY(-8px); // Se queda arriba si está seleccionado
        .icon-box {
            background: #fff;
...
// --- BOTÓN VER TODOS CORREGIDO (ROJO ESAN) ---
.btn-ver-todos {
    text-decoration: none;
    font-weight: 800;
    color: #e31140; // Color ESAN
    font-size: 0.85rem;
    border: 2px solid #e31140; // Borde ESAN
    padding: 10px 24px;
    border-radius: 50px;
    display: inline-flex;
    align-items: center;
